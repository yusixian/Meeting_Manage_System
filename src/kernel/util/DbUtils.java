...
//                System.out.println(colName);
    ...
    /**
     * 执行sql语句进行数据库存储过程的调用
     * @param cSql 待填充参数的数据库查询语句
     * @param params 准备填充的参数列表
     */
    public int executeCallableQuery(String cSql, List<Object> params) {
        try {
            cs = connection.prepareCall(cSql);
            if (cs != null) {
                for (int i = 0; i < params.size(); i++) {
            ...
            System.out.println("执行了查询语句？" + result);
            return cs.getUpdateCount();
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("在SQL存储过程调用时发生了异常");
        }
        return -1;
    }
